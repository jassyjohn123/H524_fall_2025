...
cat("Power:", round(power, 3))  # 0.345
\end{verbatim}
\end{frame}

\begin{frame}
\frametitle{Power Calculation Results}
\textbf{Result:} Power = 0.345 (34.5\%)

\vspace{0.5cm}
\textbf{Interpretation:}
\begin{itemize}
\item Only 34.5\% chance of detecting a 10 mg/dL reduction with $n=25$
\item 65.5\% chance of Type II error (missing the effect)
...
# Power = 0.345 (matches our calculation!)
\end{verbatim}
\end{frame}
...
\item $H_A: \mu_C > \mu_T$ (treatment reduces growth, one-sided)
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{Two-Sample t-Test in R}
\small
\begin{verbatim}
# Enter data
...
# Conclusion: p = 0.067 > 0.05, fail to reject H0
...
\item $\text{SE} = 2.405 \sqrt{\frac{1}{10} + \frac{1}{10}} = 1.076$ mm
\item 95\% CI: $1.7 \pm 2.101 \times 1.076 = 1.7 \pm 2.26$
...
# medication reduces blood pressure by 5.2 mmHg on average
\end{verbatim}
\end{frame}
...
\vspace{0.3cm}
\begin{itemize}
\item \textbf{Between-group variation:} How different are the group means?
\item \textbf{Within-group variation:} How variable are observations within each group?
\end{itemize}
...
\item Between-group variation $\approx$ within-group variation
\item Differences in sample means just due to chance
\end{itemize}

\textbf{If $H_A$ is true:}
\begin{itemize}
\item Between-group variation $>$ within-group variation
\item Group means genuinely different
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{ANOVA: Partitioning Variability}
...
where $x_{ij}$ = observation $j$ in group $i$, $\bar{x}_i$ = mean of group $i$, $\bar{x}$ = overall mean
\end{frame}
...
\item Check with histograms, Q-Q plots for each group
\end{itemize}

\vspace{0.3cm}
\textbf{3. Equal Variances (Homogeneity of Variance)}
\begin{itemize}
\item $\sigma_1^2 = \sigma_2^2 = \cdots = \sigma_k^2$
\item Check with Levene's test or Bartlett's test
\item Or visually: boxplots should have similar spreads
...
# Conclusion: F(2,12) = 149.4, p < 0.001
...
\begin{frame}[fragile]
\frametitle{Post-Hoc Tests in R}
\small
\begin{verbatim}
# Tukey's HSD test
TukeyHSD(model)
...
# Alternative: Pairwise t-tests with Bonferroni
...
\section{AI-Enhanced Learning for Power and Comparisons}

\AIcolors

\begin{frame}
\frametitle{AI Tools for Power Analysis and Testing}
\textbf{How AI can help this week:}
\begin{itemize}
\item \textbf{Power calculations:} Sample size determination
\item \textbf{Study design:} Choosing between paired and independent designs
\item \textbf{Test selection:} When to use t-test vs. ANOVA
\item \textbf{R code generation:} Performing power analysis and tests
\item \textbf{Assumption checking:} Verifying normality and equal variances
\item \textbf{Results interpretation:} Understanding ANOVA tables and post-hoc tests
\end{itemize}

\vspace{0.3cm}
\textbf{Example prompts:}
\begin{itemize}
\item ``Calculate required sample size to detect 10-point difference in mean with 80\% power''
\item ``Explain when to use paired vs. independent t-test with medical examples''
\item ``Generate R code for one-way ANOVA with Tukey post-hoc tests''
\item ``How do I check ANOVA assumptions in R?''
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Power Analysis}
\textbf{Prompt:} ``Calculate sample size needed to detect 5 mmHg reduction in blood pressure with power 0.80, assuming SD=15''
...
\begin{frame}[fragile]
\frametitle{AI Demo: Choosing the Right Test}
\textbf{Prompt:} ``I have blood pressure measured before and after treatment in 20 patients. Which test should I use?''
...
DO NOT use independent two-sample t-test!
- That would ignore pairing
- Would lose power
- Would give wrong standard error

The paired test is more powerful because it removes
between-subject variability and focuses on within-
subject changes.
\end{verbatim}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: ANOVA and Post-Hoc Tests}
\textbf{Prompt:} ``Perform ANOVA comparing 4 treatment groups and follow up with post-hoc tests in R''
...
\begin{frame}
\frametitle{AI Best Practices for Hypothesis Tests}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{DO:}
\begin{itemize}
\item Check test assumptions first
\item Visualize data before testing
\item Consider study design (paired?)
\item Calculate power before study
\item Interpret effect sizes, not just p-values
\item Verify AI's test selection
\end{itemize}
\end{column}
...
\textbf{DON'T:}
\begin{itemize}
\item Use wrong test type (paired vs. independent)
\item Run many tests without correction
\item Ignore violated assumptions
\item Trust AI code blindly
\item P-hack by trying many tests
\item Forget about power/sample size
\end{itemize}
\end{column}
\end{columns}

\vspace{0.3cm}
\textbf{Critical thinking checklist:}
\begin{enumerate}
\item Are observations independent or paired?
\item How many groups am I comparing?
\item Are assumptions met (normality, equal variance)?
\item What is my hypothesis (one-sided vs. two-sided)?
\item Do I need post-hoc tests?
\end{enumerate}
\end{frame}
...
\node (paired) [draw, rectangle, below of=two, yshift=-0.5cm] {Paired or Independent?};
\node (pairedt) [draw, ellipse, below left of=paired, xshift=-1cm, yshift=-0.5cm] {Paired t-test};
\node (indept) [draw, ellipse, below right of=paired, xshift=1cm, yshift=-0.5cm] {Two-sample t-test};
\node (anova) [draw, ellipse, below of=many, yshift=-0.5cm] {ANOVA};
...
\item Normal data? If not, consider non-parametric tests (Week 7)
\item Small samples with non-normal data? Use caution or transform
\end{itemize}
\end{frame}
