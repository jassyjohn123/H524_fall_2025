...
\item \textcolor{aiblue}{\textbf{Use AI tools to help with distribution problems and R code}}
\end{enumerate}
...
\begin{tcolorbox}[colback=aiblue!10,colframe=aiblue,title=Using AI Tools for Distributions]

\textbf{Good prompts for AI assistance:}
\begin{itemize}
\item ``Explain the difference between dbinom and pbinom with an example''
\item ``Show me how to calculate a 95\% confidence interval in R using t-distribution''
\item ``Create R code to simulate the Central Limit Theorem with sample size 50''
\item ``What does this error mean: Error in qnorm(2) : argument out of domain [0, 1]''
\end{itemize}

\textbf{AI can help with:}
\begin{itemize}
\item Explaining which distribution function to use
\item Debugging your R code
\item Creating visualizations
\item Understanding probability concepts
\item Suggesting alternative approaches
\end{itemize}

\textbf{Remember:}
\begin{itemize}
\item Always verify AI-generated probabilities
\item Test code on simple examples first
\item Make sure you understand the concepts, not just the code
...
\item R Documentation:
  \begin{itemize}
  \item \texttt{?dbinom} -- Binomial distribution help
  \item \texttt{?pnorm} -- Normal distribution help
  \item \texttt{?t.test} -- Confidence intervals and t-tests
  \end{itemize}
\item Quick-R: Distribution Functions (\url{https://www.statmethods.net/advgraphs/probability.html})
\item Textbook: Pagano \& Gauvreau, Chapters 7 and 8
...
\begin{enumerate}
\item \textbf{Binomial:} Use for count data with fixed n, constant p
\item \textbf{Normal:} Use for continuous data, foundation for inference
\item \textbf{Standard Error:} SD of sample mean = $\sigma/\sqrt{n}$, decreases with larger n
\item \textbf{Central Limit Theorem:} Sample means are approximately normal, regardless of population distribution
\item \textbf{Confidence Intervals:} Quantify uncertainty, use t-distribution when $\sigma$ unknown
\item \textbf{R Functions:}
  \begin{itemize}
  \item \texttt{d}* functions: density/probability mass
  \item \texttt{p}* functions: cumulative probability
  \item \texttt{q}* functions: quantiles (inverse of p)
  \item \texttt{r}* functions: random generation
  \end{itemize}
\end{enumerate}
