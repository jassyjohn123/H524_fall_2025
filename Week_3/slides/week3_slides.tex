...
\section{AI as a Learning Tool for Distributions}

\AIcolors
\begin{frame}
\frametitle{AI for Probability Distributions}
\textbf{How AI can help this week:}
\begin{itemize}
\item \textbf{Distribution calculations:} Binomial and normal probabilities
\item \textbf{Standardization:} Converting to Z-scores
\item \textbf{Simulation:} Demonstrating CLT
\item \textbf{Visualization:} Creating distribution plots
\item \textbf{Conceptual explanations:} Understanding sampling distributions
\item \textbf{R code generation:} Working with distribution functions
\end{itemize}

\vspace{0.3cm}
\textbf{Example prompts:}
\begin{itemize}
\item ``Calculate binomial probability: n=20, p=0.75, find P(X$\geq$15)''
\item ``Simulate CLT: draw 1000 samples of size 30 from exponential distribution, plot sample means''
\item ``Explain difference between standard deviation and standard error with medical example''
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Binomial Calculations}
\textbf{Prompt:} ``Create R code for binomial distribution with n=10, p=0.75''

\vspace{0.2cm}
\textbf{Claude AI Response:}
\tiny
\begin{verbatim}
# Binomial distribution: n=10, p=0.75
n <- 10
p <- 0.75
...
cat("SD:", round(std_dev, 3), "\n")

# Visualize
x <- 0:n
probs <- dbinom(x, n, p)
barplot(probs, names.arg=x, col="lightblue",
        main="Binomial(10, 0.75)")
\end{verbatim}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Normal Distribution}
\textbf{Prompt:} ``Find what cholesterol level is at 90th percentile if mean=200, SD=40''
...
# Verification: what proportion below this value?
check <- pnorm(percentile_90, mu, sigma)
cat("Proportion below:", round(check, 3), "\n")

# Visualize
x <- seq(100, 300, by=1)
y <- dnorm(x, mu, sigma)
plot(x, y, type="l", main="Cholesterol Distribution",
     xlab="Cholesterol (mg/dL)", ylab="Density")
abline(v=percentile_90, col="red", lty=2)
text(percentile_90, 0.008, "90th %ile", pos=4)
\end{verbatim}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Central Limit Theorem}
\textbf{Prompt:} ``Demonstrate CLT by sampling from exponential distribution''

\vspace{0.2cm}
\textbf{Claude AI Response:}
\tiny
\begin{verbatim}
# CLT Demonstration: Exponential population (skewed)
set.seed(123)

# Parameters
n_samples <- 1000    # Number of samples
sample_size <- 30    # Size of each sample
lambda <- 0.5        # Exponential rate parameter
...
# Theoretical values
pop_mean <- 1/lambda  # E(X) for exponential
pop_sd <- 1/lambda    # SD(X) for exponential
se <- pop_sd/sqrt(sample_size)  # SE of sample mean

# Plot
hist(sample_means, breaks=30, prob=TRUE,
     main="Sampling Distribution of Mean (n=30)",
     xlab="Sample Mean", col="lightblue")

# Overlay theoretical normal
x <- seq(min(sample_means), max(sample_means), length=100)
lines(x, dnorm(x, mean=pop_mean, sd=se), col="red", lwd=2)
legend("topright", "CLT Prediction", col="red", lty=1)
\end{verbatim}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Confidence Intervals}
\textbf{Prompt:} ``Calculate 95\% CI for mean from sample data in R''
...
\begin{frame}
\frametitle{AI Best Practices for Distributions}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{DO:}
\begin{itemize}
\item Verify probabilities with multiple methods
\item Check distribution assumptions
\item Ask for visualizations
\item Request explanation of formulas
\item Cross-check critical values
...
\textbf{DON'T:}
\begin{itemize}
\item Trust probability calculations blindly
\item Skip understanding the theory
\item Confuse different distributions
\item Ignore warnings about assumptions
\item Forget to specify parameters
\item Use without verifying results
\end{itemize}
\end{column}
\end{columns}
...
\begin{enumerate}
\item Check parameters match your problem
\item Verify probabilities sum to 1 or make sense
\item Compare AI output with hand calculations
\item Test with extreme values to check logic
\end{enumerate}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Task: Distribution Problem Solving}
\textbf{This week's AI-integrated tasks:}
\begin{enumerate}
\item Generate code to visualize binomial vs normal approximation
\item Simulate sampling distributions for different sample sizes
\item Ask AI to explain when to use Z vs t-distribution
\item Create confidence interval calculator in R
\item Debug incorrect distribution code provided by AI
\end{enumerate}
...
Mean reduction: 12.5 units, SD: 4.2 units
Create R code to:
1) Calculate 95% CI for mean reduction
2) Visualize the t-distribution
3) Show how CI changes with sample size
...
pbinom(17, n, p, lower.tail=FALSE)  # 0.608
# or: 1 - pbinom(17, n, p)
