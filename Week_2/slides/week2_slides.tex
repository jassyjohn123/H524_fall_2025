...
\item \textbf{Event:} Subset of the sample space
  ...
  \end{itemize}
\item \textbf{Probability:} Measure of likelihood [0, 1]
  \begin{itemize}
  \item $\Pr(\text{Event}) = \frac{\text{Number of favorable outcomes}}{\text{Total number of possible outcomes}}$
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{enumerate}
\item \textbf{Range:} $0 \leq \Pr(A) \leq 1$ for any event $A$
\item \textbf{Certainty:} $\Pr(S) = 1$ (something must happen)
\item \textbf{Complement:} $\Pr(A^c) = 1 - \Pr(A)$
...
\item $\Pr(A|B)$ = Probability of diabetes given patient is overweight
\end{itemize}
...
$$\Pr(A|B) = \Pr(A) \text{ or equivalently } \Pr(A \cap B) = \Pr(A) \times \Pr(B)$$
...
\begin{itemize}
\item \textbf{Sensitivity:} Pr(Positive Test $\mid$ Disease Present)
  \begin{itemize}
  \item Ability to correctly identify disease
  \item True positive rate
  \end{itemize}
\item \textbf{Specificity:} Pr(Negative Test $\mid$ Disease Absent)
  \begin{itemize}
...
\begin{itemize}
\item \textbf{Positive Predictive Value (PPV):} $\Pr(\text{Disease} \mid \text{Positive Test})$
  \begin{itemize}
  \item Probability patient has disease given positive test
  \item Depends on disease prevalence
  \end{itemize}
\item \textbf{Negative Predictive Value (NPV):} $\Pr(\text{No Disease} \mid \text{Negative Test})$
  \begin{itemize}
...
\begin{frame}
\frametitle{A Counterintuitive Example}
\textbf{HIV Screening Test with Excellent Performance:}
\begin{itemize}
\item Sensitivity = 99.5\% (detects 995 of 1000 infected people)
\item Specificity = 98.5\% (correctly identifies 985 of 1000 uninfected)
\item Population prevalence = 0.1\% (1 in 1000 people)
\end{itemize}

\vspace{0.3cm}
\textbf{Question:} If you test positive, what's the probability you have HIV?

\pause
\vspace{0.3cm}
\textbf{Approach: Consider 100,000 people tested}

\begin{center}
\small
\begin{tabular}{|l|c|c|c|}
\hline
& \textbf{HIV +} & \textbf{HIV -} & \textbf{Total} \\
\hline
\textbf{Test +} & 100 & 1,498 & 1,598 \\
\hline
\textbf{Test -} & 0 & 98,402 & 98,402 \\
\hline
...
\vspace{0.3cm}
\textbf{PPV} = $\frac{100}{1,598} = 0.063$ or \textbf{6.3\%}

\vspace{0.2cm}
\textbf{Surprising Result:} Only 6.3\% of positive tests indicate actual infection!\\
\textbf{Why?} With low prevalence, false positives (1,498) vastly outnumber true positives (100).
\end{frame}

\section{Diagnostic Tests (Ch 6.3)}

\begin{frame}
\frametitle{Understanding Odds}
\textbf{Probability vs Odds:}
\begin{itemize}
\item \textbf{Probability:} $\Pr(\text{Event}) = \frac{\text{Number with event}}{\text{Total number}}$
\item \textbf{Odds:} $\text{Odds}(\text{Event}) = \frac{\text{Number with event}}{\text{Number without event}}$
\end{itemize}

\vspace{0.5cm}
\textbf{Relationship:} $\text{Odds} = \frac{p}{1-p}$ where $p$ is the probability

\vspace{0.5cm}
\textbf{Example: Risk of lung cancer in smokers}
\begin{itemize}
\item 60 smokers develop lung cancer, 440 don't
\item \textbf{Risk (probability):} $\frac{60}{500} = 0.12$ (12\%)
\item \textbf{Odds:} $\frac{60}{440} = 0.136$ (or ``60 to 440'' or ``3 to 22'')
\end{itemize}

\vspace{0.3cm}
\textbf{Key insight:} When probability is small ($< 0.10$), odds $\approx$ probability
\end{frame}
...
\item $\Pr(X = 18) = \binom{20}{18} (0.90)^{18} (0.10)^2$
\item $\Pr(X = 18) = 190 \times 0.1501 \times 0.01 = 0.285$
\end{itemize}
...
\item Look up in Z-table: $\Pr(Z < 1.33) = 0.9082$
\item Therefore: $\Pr(BP > 140) = 1 - 0.9082 = 0.0918$
\end{enumerate}
...
\begin{itemize}
\item Pr(disease) = 125/50,000 = 0.0025 (0.25\%)
\item Pr(death $\mid$ disease) = 15/125 = 0.12 (12\%)
\item Pr(death and disease) = 15/50,000 = 0.0003 (0.03\%)
\end{itemize}
...
\begin{frame}[fragile]
\frametitle{AI Task: Probability Problem Solving}
