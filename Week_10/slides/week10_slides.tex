...
\section{AI for Statistical Communication}

\AIcolors
\begin{frame}
\frametitle{AI for Presenting Statistical Results}
\textbf{How AI can help with your project presentations:}

\vspace{0.3cm}
\begin{itemize}
\item \textbf{Clarifying explanations:} Translate technical results for non-experts
\item \textbf{Visualization code:} Generate R code for publication-quality plots
\item \textbf{Interpretation guidance:} Help explain what results mean in context
\item \textbf{Checking logic:} Verify your statistical reasoning
\item \textbf{Writing assistance:} Draft results sections
\item \textbf{Q\&A preparation:} Anticipate questions about your analysis
\end{itemize}

\vspace{0.3cm}
\textbf{Example prompts:}
\begin{itemize}
\item ``Explain what R-squared means to someone without statistics training''
\item ``Create a scatterplot with regression line and 95\% CI bands in R''
\item ``How do I interpret a slope of 2.5 with 95\% CI (1.2, 3.8)?''
\item ``What are potential limitations of simple linear regression?''
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Explaining Results}
\textbf{Prompt:} ``I found slope = 1.2 (SE = 0.3) for BMI predicting blood pressure. Explain this to a doctor.''

\vspace{0.3cm}
\textbf{Claude AI Response:}
\small
\begin{verbatim}
This means that for each 1-point increase in BMI,
we expect systolic blood pressure to increase by
1.2 mmHg on average.

The standard error (0.3) tells us the precision of
this estimate. A 95% confidence interval would be:
1.2 ± 1.96(0.3) = (0.6, 1.8) mmHg

Clinically, this suggests that a patient with BMI
of 30 would have blood pressure about 12 mmHg
higher than a patient with BMI of 20, all else equal.

This is a statistically significant and clinically
meaningful association, though it doesn't prove that
reducing BMI will lower blood pressure (correlation
vs causation).
\end{verbatim}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Creating Visualizations}
\textbf{Prompt:} ``Create an R plot showing regression line with both confidence and prediction bands''
...
x_seq <- seq(min(mydata$x), max(mydata$x), length=100)

# Confidence interval
pred_ci <- predict(model, newdata=data.frame(x=x_seq),
                   interval="confidence", level=0.95)

# Prediction interval
pred_pi <- predict(model, newdata=data.frame(x=x_seq),
                   interval="prediction", level=0.95)

# Plot
plot(mydata$x, mydata$y, pch=16, col="gray50",
     xlab="X", ylab="Y", main="Regression with Intervals")
lines(x_seq, pred_ci[,"fit"], col="blue", lwd=2)
lines(x_seq, pred_ci[,"lwr"], col="blue", lty=2)
lines(x_seq, pred_ci[,"upr"], col="blue", lty=2)
lines(x_seq, pred_pi[,"lwr"], col="red", lty=2)
lines(x_seq, pred_pi[,"upr"], col="red", lty=2)
legend("topleft", c("Data", "Fit", "95% CI", "95% PI"),
       col=c("gray50","blue","blue","red"), lty=c(NA,1,2,2),
       pch=c(16,NA,NA,NA))
\end{verbatim}
\end{frame}

\begin{frame}
\frametitle{AI Best Practices for Presentations}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{DO:}
\begin{itemize}
\item Use AI to clarify complex concepts
\item Ask for multiple visualization options
\item Request interpretation help
\item Generate example explanations
\item Check your statistical reasoning
\item Get help with R code
\end{itemize}
\end{column}
...
\textbf{DON'T:}
\begin{itemize}
\item Let AI write entire presentation
\item Use AI explanations without understanding
\item Trust AI for statistical decisions
\item Copy AI text verbatim
\item Skip checking AI's work
\item Ignore your own judgment
\end{itemize}
\end{column}
\end{columns}

\vspace{0.5cm}
\textbf{Remember:} AI is a tool to enhance YOUR work, not replace it

\vspace{0.3cm}
\textbf{For final projects:}
\begin{itemize}
\item You must understand everything you present
\item Be prepared to answer questions independently
\item Acknowledge AI assistance if used
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{AI Task: Preparing Your Presentation}
\textbf{Use AI to help with these tasks:}

\vspace{0.3cm}
\begin{enumerate}
\item \textbf{Simplify technical language}
  \begin{itemize}
  \item Ask AI to rephrase statistical jargon
  \end{itemize}

\item \textbf{Create clear visualizations}
  \begin{itemize}
  \item Get R code for professional plots
  \end{itemize}

\item \textbf{Anticipate questions}
  \begin{itemize}
  \item Ask ``What questions might someone ask about this analysis?''
  \end{itemize}

\item \textbf{Check interpretations}
  \begin{itemize}
  \item Verify your understanding is correct
  \end{itemize}

\item \textbf{Draft explanations}
  \begin{itemize}
  \item Get help wording conclusions
  \end{itemize}
\end{enumerate}

\vspace{0.3cm}
\textbf{Example:} ``Review my regression analysis and suggest improvements to the presentation''
\end{frame}
