...
\section{AI as a Learning Tool for Regression}

\AIcolors
\begin{frame}
\frametitle{AI for Regression Analysis}
\textbf{How AI can help this week:}
\begin{itemize}
\item \textbf{Code generation:} Create regression models in R
\item \textbf{Interpretation:} Explain regression output
\item \textbf{Diagnostics:} Generate residual plots and check assumptions
\item \textbf{Visualization:} Create scatter plots with regression lines
\item \textbf{Predictions:} Calculate predictions and intervals
\item \textbf{Conceptual help:} Clarify difference between correlation and regression
\end{itemize}

\vspace{0.3cm}
\textbf{Example prompts:}
\begin{itemize}
\item ``Create R code to fit linear regression of weight on height, plot results''
\item ``Explain how to interpret regression slope in context of my data''
\item ``Generate residual plots to check regression assumptions''
\item ``Calculate 95\% prediction interval for new observation''
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Fitting Regression Model}
\textbf{Prompt:} ``Fit linear regression of systolic BP on age in R''

\vspace{0.2cm}
\textbf{Claude AI Response:}
\tiny
\begin{verbatim}
# Sample data: age and systolic BP
...
# Interpretation
cat("Interpretation: For each additional year of age,\n")
cat("systolic BP increases by", round(coef(model)[2], 2), "mmHg\n\n")

# R-squared
cat("R-squared:", round(summary(model)$r.squared, 3), "\n")
cat("Age explains", round(100*summary(model)$r.squared, 1),
    "% of variability in SBP\n")
\end{verbatim}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Visualization}
\textbf{Prompt:} ``Create scatter plot with regression line and prediction interval''
...
new_ages <- seq(min(age), max(age), length.out = 100)
pred_interval <- predict(model,
                         newdata = data.frame(age = new_ages),
                         interval = "prediction",
                         level = 0.95)

# Plot prediction bands
lines(new_ages, pred_interval[,"lwr"], col = "gray", lty = 2, lwd = 2)
lines(new_ages, pred_interval[,"upr"], col = "gray", lty = 2, lwd = 2)

# Add legend
legend("topleft",
       legend = c("Observed data", "Regression line", "95% PI"),
       col = c("blue", "red", "gray"),
       lty = c(NA, 1, 2),
       pch = c(19, NA, NA),
       lwd = 2)
...
\textbf{Prompt:} ``Create diagnostic plots to check regression assumptions''
...
cat("If p > 0.05, residuals appear normally distributed\n")
\end{verbatim}
\end{frame}

\begin{frame}[fragile]
\frametitle{AI Demo: Making Predictions}
\textbf{Prompt:} ``Predict systolic BP for ages 45, 55, and 65 with confidence intervals''

\vspace{0.2cm}
\textbf{Claude AI Response:}
\tiny
\begin{verbatim}
# Create data frame with new ages
new_data <- data.frame(age = c(45, 55, 65))

# Predict mean SBP (confidence interval)
conf_predictions <- predict(model,
                            newdata = new_data,
                            interval = "confidence",
                            level = 0.95)

# Predict individual SBP (prediction interval)
pred_predictions <- predict(model,
                            newdata = new_data,
                            interval = "prediction",
                            level = 0.95)

# Display results
cat("Predictions for systolic BP:\n\n")
for(i in 1:nrow(new_data)) {
  cat("Age", new_data$age[i], "years:\n")
  cat("  Predicted SBP:", round(conf_predictions[i,"fit"], 1), "mmHg\n")
  cat("  95% CI for mean:",
      round(conf_predictions[i,"lwr"], 1), "to",
      round(conf_predictions[i,"upr"], 1), "mmHg\n")
  cat("  95% PI for individual:",
      round(pred_predictions[i,"lwr"], 1), "to",
      round(pred_predictions[i,"upr"], 1), "mmHg\n\n")
}
\end{verbatim}
\end{frame}

\begin{frame}
\frametitle{AI Best Practices for Regression}
\begin{columns}
\begin{column}{0.5\textwidth}
\textbf{DO:}
\begin{itemize}
\item Visualize data before modeling
\item Check all assumptions
\item Examine residual plots
\item Interpret in context
\item Report confidence intervals
\item Verify AI predictions
\end{itemize}
\end{column}
...
\textbf{DON'T:}
\begin{itemize}
\item Skip diagnostic plots
\item Ignore assumption violations
\item Extrapolate beyond data
\item Confuse correlation with causation
\item Blindly trust AI output
\item Forget biological plausibility
\end{itemize}
\end{column}
\end{columns}
...
\begin{enumerate}
\item Check scatter plot makes sense
\item Verify regression coefficients are reasonable
\item Examine residual plots for patterns
\item Confirm $R^2$ and $p$-values match interpretation
\item Test predictions against known values
\end{enumerate}
\end{frame}

\begin{frame}
\frametitle{AI for Learning Regression Concepts}
\textbf{Effective AI prompts for understanding:}

\vspace{0.3cm}
\begin{itemize}
\item ``Explain the difference between correlation and regression with a medical example''
\item ``Why do we check residual plots? What patterns indicate problems?''
\item ``What's the difference between confidence interval and prediction interval?''
\item ``When should I use Spearman correlation instead of Pearson?''
\item ``How do I interpret a regression slope in my specific context?''
\end{itemize}
...
\item Interpretation of output
\item Choosing appropriate methods
\item Troubleshooting problems
\item Generating example datasets
\end{itemize}

\vspace{0.3cm}
\textbf{Remember:} AI is a tutor, not a replacement for understanding. Always verify and think critically!
\end{frame}
