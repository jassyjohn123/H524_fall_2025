...
\textbf{Statistics} provides methods and tools for collecting, analyzing, interpreting, and drawing inferences from sample data.

\vspace{0.5cm}
\textbf{Two main branches:}
\begin{itemize}
\item \textbf{Descriptive Statistics} -- describe and summarize data
\item \textbf{Inferential Statistics} -- make inferences about populations from samples
\end{itemize}

\vspace{0.5cm}
\textbf{Biostatistics:} Application of statistics to biomedical and health data
\begin{itemize}
\item Broad interpretation of "Bio"
\item Applied to drug testing, air pollution studies, nutrition, etc.
\item \textit{Example questions:} Is elevated air pollution associated with decreased lung function? Does a new treatment significantly lower risk compared to standard treatment?
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Population vs. Sample}
\textbf{Population:} The totality of subjects under study
\begin{itemize}
\item Often impossible or impractical to study entire population
\item Contains the true \textit{parameters} we want to learn about
\end{itemize}

\vspace{0.5cm}
\textbf{Sample:} The subset of the population actually studied
\begin{itemize}
\item Must be representative of the population
\item Provides \textit{statistics} that estimate population parameters
\end{itemize}

\vspace{0.5cm}
\textbf{Key Concept:} Statistical inference involves making assertions about population \textit{parameters} based on sample \textit{statistics}, subject to sampling error.
\end{frame}
...
\begin{frame}
\frametitle{Empirical Rule (68-95-99.7 Rule)}
\textbf{When data are symmetric and unimodal (bell-shaped):}

\vspace{0.3cm}
\begin{itemize}
\item Approximately \textbf{68\%} of observations fall within \textbf{1 standard deviation} of the mean
\item Approximately \textbf{95\%} fall within \textbf{2 standard deviations} of the mean
\item Approximately \textbf{99.7\%} fall within \textbf{3 standard deviations} of the mean
\end{itemize}

\vspace{0.3cm}
\textbf{Application:} Helps identify unusual observations and understand data distribution patterns

\vspace{0.3cm}
\textbf{Example:} If blood pressure has mean = 120 mmHg and SD = 15 mmHg:
\begin{itemize}
\item 68\% of people have BP between 105-135 mmHg
\item 95\% have BP between 90-150 mmHg
\end{itemize}
\end{frame}
...
\item \textbf{Interquartile Range (IQR):} Q3 - Q1
  ...
  \item Resistant to outliers
  \end{itemize}
\item \textbf{Five-Number Summary:} Min, Q1, Median, Q3, Max
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Data Collection and Sampling}
\textbf{Representative Sample:} Distribution of values in sample mimics population distribution

\vspace{0.3cm}
\textbf{Common Sampling Methods:}
\begin{itemize}
\item \textbf{Simple Random Sample (SRS):} Every group of size n has same chance of selection
\item \textbf{Stratified Random Sample:} Divide population into strata, then SRS from each stratum
\item \textbf{Cluster Sampling:} Divide into clusters, select random clusters, study everyone in selected clusters
\item \textbf{Sample of Convenience:} Easy to obtain but may not be representative
\end{itemize}

\vspace{0.3cm}
\textbf{Key Point:} Non-representative samples can be very misleading due to sampling bias
\end{frame}
...
\vspace{0.5cm}
$$\text{SMR} = \frac{\text{Observed events}}{\text{Expected events}}$$

\textbf{SMR} = Standardized Mortality Ratio
\begin{itemize}
\item SMR $>$ 1: Higher than expected
\item SMR $<$ 1: Lower than expected
\item SMR $=$ 1: Same as expected
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \textbf{Central tendency:} Average BP (130.3) is in "high" range (AHA guidelines: <120 normal, 120-129 elevated, 130+ high)
\item \textbf{Variability:} Large SD (18.7) indicates substantial individual differences
\item \textbf{Distribution:} Mean < median suggests slight left skew (some low outliers)
...
mean(bp_data$systolic_bp)     # Mean: 130.3 mmHg
sd(bp_data$systolic_bp)       # SD: 18.7 mmHg
